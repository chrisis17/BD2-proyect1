...
public:
            ...
            if (bucket.isFull())
            {
                ...
                if (bucket.hasMaxLocalHeight())
                {
                    ...
                    if (bucket.isFull())
                    {
                        ...
                        write(f, new_bucket);accesos++;
                        f.seekp(prev_pos, ios::beg);
                        write(f, bucket);accesos++;
                    }
                    else
                    {
                        //insertar
    ...
    void showRecords(){
        int cont = 1;
        cout << setw(4) << "Nº" << setw(5) << "ID" << setw(10) << "FechaCorte" << setw(10) << "Fecha" << setw(6) << "Hora" << setw(12) << "Departamento" << setw(6) << "Codigo" << setw(5) << "Kilom" << setw(12) << "Modalidad" << setw(3) << "Fal" << setw(3) << "Her" << endl;
        set<long> visited;
        fopen(fileName);
        isfopen
        {
